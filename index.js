...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Processando seu CORNO ⌛',
				success: '✔️ Funcionando ✔️',
                                levelon: '❬ ✔ ❭ *Sistema de Level ativado pelo ADM gay*',
				leveloff: ' ❬ X ❭  *Oporra Level desativado*',
				levelnoton: '❬ X ❭ *A porra do ADM não ativou o sistema de level*',
				levelnol: '*LEVEL KAKAK MASIH* 0 °-°',
				error: {
					stick: '[❗] Deu erro ao converter essa merda em figurinha ❌',
					Iv: '❌ Link Invalido ❌'
				},
				only: {
					group: '[❗] Tu é burro, esse comando so funciona em grupos! ❌',
					ownerG: '[Este comando só pode ser usado pelo proprietário do grupo!❗] ! ❌',
					ownerB: '[❗]Este comando só pode ser usado pelo Deus Saiko ! ❌',
					admin: '[❗] Ala o membro comum querendo usar comando dos ADMS, seu inferior! ❌',
					Badmin: '[❗] Passa o ADM né carai, sem ele não posso executar esse comando! ❌',
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`*「 LEVEL UP 」*\n\n➸ *Name*: ${sender}\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *Level*: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParabéns agora você é um gay mais avançado!! 🎉🎉`)
                }
            } catch (err) {
                console.error(err)
            }
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, sinta se feliz ahora você é um Deus no grupo :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(` Enviar o pedido${prefix}snowwrite texto1|texto2, contoh ${prefix}snowwrite aqulzz|galuh`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply(' Onde está o url cabeça de cunha?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                break
				case 'blocklist':
					teks = 'Está é a lista de números bloqueados:\n'
                   ...
                   case 'chatlist':
					client.updatePresence(from, Presence.composing)  
					teks = 'Está é a lista de números de chat :\n'
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem ? Parece que é burro.', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista de gays do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'meme':
					...
					buffer = await getBuffer(`https://br.ifunny.co/${meme.hash}.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://br.ifunny.co/${memein.hash}.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'block':
					client.updatePresence(from, Presence.composing) 
				...
				case 'tagall':
					...
					mentions('╔══✪〘 Chamando todos os BOIS 〙✪══\n╠➥'+teks+'╚═〘 - - - - 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Menção a todos os CORNOS 〙✪══\n╠➥'+teks+'╚═〘 - - - - 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Convocando todos os GAYS 〙✪══\n╠➥'+teks+'╚═〘 - - - - - 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Vem geral ver o GAY do rudi 〙✪══\n╠➥'+teks+'╚═〘 - - - - - 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'chord':
					if (args.length < 1) return reply('Onde está o título da música seu gay')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Qual o nome da musica amigue ?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, para tirar esse gay:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Mete o pé daqui não gosto de gays😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, "Commando errado", text, { quoted: mek })
		           if (stdout) {
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isRegister) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('❌ responda a um adesivo oni-chan ❌')
					reply(mess.wait)
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('❌ Responda a um vídeo ❌')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('Onde está o texto my lord ?')
                      if (!isRegister) return reply(mess.only.daftarB)
                      if (isLimit(sender)) return reply(ind.limitend(pusname))
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					setTimeout( () => {
					client.sendMessage(from, '_majs 10 segundos…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_majs 20 segundos_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_mais 30 segundos_…', text) // ur cods
					}, 1000) // 1000 = 1s,
					setTimeout( () => {
					...
					if (args.length < 1) return reply(' digite 1 para ativar ADM seu lindo')
					if (Number(args[0]) === 1) {
					...
					fakta = body.slice(1)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					const fakta =['A massa da Terra atinge 5.972.190.000.000.000.000.000.000 kg.  Mesmo sendo tão pesado, o fato é que a Terra tem uma velocidade de 107.281 km por hora para circundar o sol.  É muito rápido, não é? ',' A massa pesada da Terra é dominada pela poeira espacial e pode ser reduzida por um gás como o hidrogênio, que diminui três quilos a cada segundo.  Este fato único mostra que a Terra perderá 95 mil toneladas de massa a cada ano ',' Em 2018 a população humana está estimada em 7,6 bilhões de pessoas.  Embora a Terra esteja repleta de humanos, um fato único revela que os humanos não influenciam a massa da Terra.  Isso ocorre porque os humanos são formados a partir de átomos na forma de 65 por cento de oxigênio, 18,5 por cento de carbono e 9,5 por cento de hidrogênio. ',' A terra é preenchida com 70 por cento de água, por isso é natural que a Terra seja chamada de planeta da água.  O oceano mais profundo do planeta é a Fossa das Marianas, com 10.994 metros de profundidade.  O fato único por trás da Fossa das Marianas é que se você colocar o Monte Everest lá, seu pico mais alto ainda estará 1,6 quilômetros abaixo do nível do mar! ',' O fato é que a terra onde os humanos vivem é apenas um por cento de todo o planeta Terra.  Este fato único mostra que ainda existem muitas partes da terra que possuem o mistério da vida.  Interessado em explorar para descobrir mistérios, bem como fatos únicos em outras partes da terra ',' Há uma cidade na Rússia cujas ruas são cuidadosamente organizadas com um palácio projetado como um magnífico xadrez.  O acordo foi planejado por um presidente obcecado por xadrez chamado Kirsan Ilyumzhinov. ',' Você conhece algum fato único sobre mussarela feita com leite de búfala em vez de leite de vaca?  Cerca de 500 anos atrás, na Campânia, Itália, a mussarela foi feita pela primeira vez com leite de búfala.  O fato único por trás do uso do leite de búfala é porque o teor de proteína do leite de búfala é 10-11% maior do que o do leite de vaca. ',' Bali tem um fato único porque tem muitos feriados contribuídos por vários feriados religiosos importantes.  Quase todos os feriados religiosos, todos terão um feriado.  Alguns dos feriados incluem galungan, kuningan, nyepi, pagerwesi, saraswati e uma série de outras cerimônias importantes, como piodalan (pujawali). ',' A capital Jacarta tem muitos encantos, bem como fatos únicos que você pode não saber .  Antes de receber o nome de Jacarta, a capital da Indonésia sofreu várias mudanças de nome.  Os nomes das capitais da Indonésia, de acordo com a ordem de mudança, são Sunda Kelapa, Jayakarta, Batavia, Betawi, Jacatra, Jayakarta e Jacarta. ',' Em 1952 as janelas dos aviões foram projetadas em um formato quadrado, mas seu uso foi considerado com defeito para que não fossem reaplicados.  As janelas redondas são projetadas para compensar a diferença na pressão do ar dentro e fora da aeronave para evitar falhas estruturais que podem levar a quedas de aviões. ',' A dieta principal dos mosquitos machos e fêmeas geralmente é néctar e substâncias doces, que são principalmente obtidas das plantas.  No entanto, os mosquitos precisam de proteína adicional para botar ovos, que podem ser obtidos de humanos, enquanto os mosquitos machos não precisam de substâncias proteicas adicionais para botar ovos. ',' A ponte suramadu (surabaya-madura) é a ponte mais longa do Sudeste Asiático (5438 m) . ',' O riso e a felicidade aumentam a imunidade, especialmente a produção de células assassinas naturais que ajudam a proteger o corpo contra doenças ',' As baratas peidam a cada 15 minutos e continuam a emitir gás metano (peidos) por 18 horas após a morte. ',' Aplicar cal pode clarear os joelhos / cotovelos do negro. ',' A energia produzida por um furacão (tufão) por 10 minutos é maior que a energia de uma bomba durante a guerra ',' Os únicos sentidos humanos que não funcionam durante o sono são o sentido do olfato. ',' Os astronautas estão proibidos de comer leguminosas antes de ir para o espaço sideral.  Porque pode fazer com que peidem facilmente.  E o gás de peido é muito prejudicial para seus trajes espaciais. ',' Só nos EUA, os gatos matam bilhões de animais em um único ano.  Eles são responsáveis ​​pela morte de 1,4 - 73,7 bilhões de pássaros e 6,9 ​​- 20,7 bilhões de mamíferos a cada ano.  Além disso, até agora eles eliminaram completamente um total de 33 espécies do mundo. ',' Os tubarões perdem mais de 6.000 dentes todos os anos e novos dentes crescem em 24 horas. ',' As formigas podem levantar 50 vezes seu peso . ',' A boca produz 1 litro de saliva todos os dias. ',' Um caracol pode dormir 3 anos ',' As baratas podem viver 9 dias sem cabeça e morrerão de fome ',' Os olhos de um avestruz são maior que seu cérebro']
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					if (tels.ength > 10) return reply('Texto longo ate pelo menos 9 caracteres')
					reply(mess.wait)
				...
				case 'logowolf':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSO ❭ NSFW está ativo no grupo ADEMIR ')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ Oporra NSFW desativado com sucesso')
					} else {
						reply('digite 1 para ativar, 0 para desativar o recurso '')
					}
					break	
                ...
                if (!isUser) return reply(mess.only.userB)
					...
					break
                case 'truth':
					const trut =['Você já gostou de alguém?  quanto tempo? ',' Se você pode ou se quiser, na gc / fora da gc com quem você fará amizade? (você pode ser diferente / do mesmo sexo) ',' qual é o seu maior medo? ',' você já gostava de alguém e sentia essa pessoa como você também? ',' Qual é o nome do seu amigo ex-namorado de que você gostava secretamente? ',' Você já roubou o dinheiro da sua mãe ou do pai?  O motivo? ',' O que te faz feliz quando você está triste ',' você já amou não correspondido?  se alguma vez com quem?  como você se sente, brou? ',' você foi traído por pessoas? ',' a coisa mais temida ',' quem é a pessoa mais influente em sua vida ',' que orgulho você teve este ano ',' quem é a pessoa que pode te deixar com tesão ',' quem é a pessoa que já te deixou com tesão ',' (bgi que é muçulmana) nunca orou o dia todo? ',' Quem é o mais próximo do seu tipo de parceiro ideal aqui ',' quem gosta de brincar? ',' quem já rejeitou as pessoas?  o motivo por quê? ',' Mencione o incidente que te machucou e que você ainda lembra ',' o que você conquistou este ano? ',' Qual era o seu pior hábito na escola?']
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
					client.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
                                case 'dare':
					const dare =['Envie uma mensagem para o seu ex e diga "Eu ainda gosto de você ',' ligue para o Crush / namorada agora e ss para o jogador ',' papai para um dos membros do grupo ',' Diga" VOCÊ É TÃO BONITA, NÃO É FODIDO "para pessoal ',' ss recente chamada whatsapp ',' drop emoticon "ðŸ¦„ ðŸ'¨ "cada digitação no gc / pc por 1 dia ',' enviar nota de voz diz posso te ligar, baby? ',' largar música / citação citação, continue marcando membro adequado para a citação ',' use uma foto do Sule até 3 dias ',' digite no idioma local 24 horas ',' mude o nome para "Sou uma criança fofa, luna" por 5 horas ' , 'bate-papo para entrar em contato com o pedido de acordo com% bateria você, continue dizendo a ele "tive sorte em ter você" ', 'bate-papo ex e diga" eu te amo, pgn de volta', 'gravação de voz ler surah al-kautsar', 'disse "eu tenho uma queda por você, quer ser meu namorado ou não?" para o sexo oposto que você conversou pela última vez (entregar em wa / tele), espere que ele responda, se ss cair aqui', 'diga sua namorada digite! ',' tire / poste fotos de namorada / paquera ',' gritou gajelas e envie usando vn aqui ',' faça papinha e envie para um de seus amigos  ',' envie sua foto com uma legenda, sou uma criança adotada ',' gritou com palavras ásperas enquanto Vn então a enviou aqui ',' gritou "anjimm gabutt anjimmm" na frente de sua casa, mude o nome para "BOWO" por 24 horas ',Finja estar possuído, por exemplo: sendo possuído por maung, possuído por um gafanhoto, possuído por uma geladeira, etc.']
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    ...
                    reply(' *Digite o comando 1 para ativar, 0 para desativar * \ n * Exemplo: $ {prefix} leveling 1*')
                }
            break
                                case 'infogempa':
                                ...
                                case 'nsfwtrap':
                                        try{
                                                if (!isNsfw) return reply('❌ *NSFW DESATIVADO SEU TARADO* ❌')
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
			    ...
			    if (!isUser) return reply(mess.only.userB)
                    ...
                    neonlogo = await getBuffer(`https://arugaz.my.id/api/textpro/neonlight?text=${teks}`, {method: 'get'})
				...
				if (!isRegister) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('Ta AFK por que irmão ?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto einh \ nExemplo: $ {prefix} shadow RudiGay')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=shadow&text=${text1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'onde está o texto um \ nExemplo: $ {prefix} shadow RudiGay '..'})
                                        await limitAdd(sender)
                                        break
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('Onde está o texto \nContoh: ${prefix}burnpaper RudiGay')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto um \ nExemplo: $ {prefix} café RudiGay')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('cade o texto moleque\nContoh: ${prefix}woodblock Arara baiano')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=wood_block&text=${teks1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui esta sua foto...'})
                                        await limitAdd(sender)
                                        break
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('Onde esta o texto garai\nContoh: ${prefix}qowheart Pablo Lindo')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Pega Aqui sua foto..'})
                                        await limitAdd(sender)
                                        break
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('cade o texto mané\nContoh: ${prefix}mutgrass NazwaCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Toma aki sua foto...'})
                                        await limitAdd(sender)
                                        break
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('E o texto cade\nContoh: ${prefix}undergocean NazwaCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=underwater_ocean&text=${teks1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ta pronto pega...'})
                                        await limitAdd(sender)
                                        break
                                case 'woodenboards':
                                        var gh = body.slice(13)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('esquecei de por o texto\nContoh: ${prefix}woodenboards NazwaCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui meu lindo dua foto...'})
                                        await limitAdd(sender)
                                        break
                                case '8bit':
                                        var gh = body.slice(5)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Receba sua foto...'})
                                        await limitAdd(sender)
                                        break
                                case 'randomkpop':
                                        gatauda = body.slice(6)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Toma esse lixo, estou a vomitar...'})
                                        await limitAdd(sender)
                                        break
                                case 'fml2':
                                        ...
                                        break
                                case 'tiktok':
					if (args.length < 1) return reply('Esqueceu dd botar a URL da desgraca do TIKO TEKO seu símio?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'simih':
					...
					if (args.length < 1) return reply('digite 1 para ativar o recurso')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo SIMestá ativo')
						samih.push(from)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					pubglogo = `${body.slice(7)}`
					pubglogo1 = pubglogo.split("/")[0];
					pubglogo2 = pubglogo.split("/")[1];
					...
					client.sendMessage(from, pubglogo, image, {quoted: mek})
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'lancei a braba...'})
                                        await limitAdd(sender)
                                        break
                                case 'katabijak':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'bal':
                                        ...
                                        break
                                case 'buylimit':
                                        if (args.length < 1) return reply('Qual é o limite que você quer comprar, man ?  Certifique-se de ter dinheiro suficiente !  \ n \ nComo verificar dinheiro: $ {prefix} bal '')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'event':
                                        ...
                                        if (Number(args[0]) === 1) {
                                                ...
                                                reply('*❬ SUCESSO ❭ Evento ativado neste grupo*')
                                        } else if (Number(args[0]) === 0) {
                                                event.splice(from, 1)
                                                fs.writeFileSync('./database/json/event.json', JSON.stringify(event))
                                                reply('*❬ SUCESSO ❭ Evento desativado no grupo*')
                                        } else {
                                                reply(ind.satukos())
                                        }
                                        break
                                ...
                                case 'mining':
                                        ...
                                        if (isOwner) {
                                                ...
                                                reply(`porque você é o nosso dono da equipe bot que está enviando $ {one} Xp para você`)`)
                                        } else {
			                ...
			                if (!isBotGroupAdmins) return reply(ind.badmin())
                                        ...
                                        client.sendMessage(from, 'Sucesso ao alterar o nome do grupo', text, {quoted: mek})
				        break
                                case 'setdesc':
                                        if (!isGroup) return reply(ind.groupo())
			                if (!isGroupAdmins) return reply(ind.admin())
			         	if (!isBotGroupAdmins) return reply(ind.badmin())
                                        ...
                                        client.sendMessage(from, 'Sucesso ao alterar descrição do grupo', text, {quoted: mek})
					break
			        case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                                        if (!isUser) return reply(mess.only.daftarB)
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[PERIGO]','red'), 'Comando não registrado do gay do', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
